...
namespace TrabFinalCg
{
    class Objecto
    {
        ...
        //aplicar transformações ao objecto
        ...
        private ArrayList geraFaces(ArrayList verticesTransf, int cam)
        {
            ...
            for (int i = 0; i < numvPorFace.Count; i++)
            {
                ...
                if(ocultas == 0){
                    nFacesT++;
                    normal = f.calcNormal();
                    //System.Windows.Forms.MessageBox.Show("Nº Faces: " + normal.z);
                    if (normal.z < 0)
                        faces.Add(f);
                }
                if (ocultas == 1){
           ...
           for (int i = faces.Count - 1; i >= 1; i--)
            {
                ...
                for (int j = 0; j < i; j++)
                {
                    if (((Face)faces[j]).calcZMedio() < ((Face)faces[j + 1]).calcZMedio())
                    {
                        aux = (Face)faces[j];
                        faces[j] = (Face)faces[j + 1];
                        faces[j + 1] = aux;
        ...
        public ArrayList desenha(Graphics g, float translacaoX, float translacaoY, float translacaoZ, float rodaX, float rodaY, float rodaZ, float camara, float escala, int cam, int esconderOcultas)
        {
          ArrayList res = new ArrayList();

          ocultas = esconderOcultas;
          ...
          ArrayList faces=geraFaces(vTransf, cam); // gera as novas faces com base nos novos vértices
          nVertices = vTransf.Count;
          nFacesV = faces.Count;
          res.Add(nFacesV);
          res.Add(nVertices);
          res.Add(nFacesT);
          ...
          {
            Face f = (Face)faces[i];
            //System.Windows.Forms.MessageBox.Show("" + ((Face)faces[i]).calcZMedio());
